...
const CATEGORIES: { value: KbArticleCategory; label: string }[] = [
...
const STATUSES: { value: KbArticleStatus; label: string }[] = [
...
const articleSchema = z.object({
  ...
  category: z.enum(["runbook", "troubleshooting", "faq", "sop", "configuration", "architecture", "general"]),
...
interface ArticleFormProps {
  defaultValues?: KbArticleFormData;
  onSubmit: (data: KbArticleFormData) => void | Promise<void>;
  isEditing?: boolean;
}
...
export function ArticleForm({ defaultValues, onSubmit, isEditing }: ArticleFormProps) {
  const {
    register,
    handleSubmit,
    control,
    formState: { errors, isSubmitting },
  } = useForm<ArticleSchemaData>({
  ...
  const handleFormSubmit = async (data: ArticleSchemaData) => {
    await onSubmit(data as KbArticleFormData);
  };
        ...
        <div className="space-y-4">
          <Card>
            ...
            <CardContent className="space-y-4">
              {/* Category */}
              <div className="space-y-2">
                <LabelWithTooltip tooltip="Runbook: step-by-step procedures. Troubleshooting: diagnostic guides. FAQ: common questions. SOP: standard operating procedures. Configuration: system settings docs. Architecture: system design docs. General: everything else.">
                  Category
                </LabelWithTooltip>
                <Controller
                  name="category"
